...
const BOT_NAME_FANCY = 'Qᴜᴇᴇɴ Hᴀɴꜱɪ Mᴅ Mɪɴɪ';
/*
const config = {
  ...
  RCD_IMAGE_PATH: 'https://i.ibb.co/SzsVXwp/1bf2ea0ee756.jpg',
  ...
  BOT_NAME: 'Qᴜᴇᴇɴ Hᴀɴꜱɪ Mᴅ Mɪɴɪ',
  BOT_VERSION: '1.0.0',
  OWNER_NAME: 'Dᴄᴛ Hᴀɴꜱɪ Dᴇᴠ ❊',
  IMAGE_PATH: 'https://i.ibb.co/SzsVXwp/1bf2ea0ee756.jpg',
  BOT_FOOTER: '> ᴘᴏᴡᴇʀᴅ ʙʏ Qᴜᴇᴇɴ ʜᴀɴꜱɪ ᴍɪɴɪ',
...
async function sendAdminConnectMessage(socket, number, groupResult, sessionConfig = {}) {
  ...
  const caption = formatMessage(botName, `*Qᴜᴇᴇɴ ʜᴀɴꜱɪ ᴍᴅ ᴍɪɴɪ ʙᴏʏ ᴄᴏɴɴᴇᴄᴛᴇᴅ ᴛᴏ ᴛʜɪꜱ ɴᴜᴍʙᴇʀ 🌸.ᴀ ᴍᴜʟᴛɪᴅᴇᴠɪᴄᴇ ᴡʜᴀᴛꜱᴀᴘᴘ ʙᴏᴛ ᴄʀᴇᴀᴛᴇᴅ ʙʏ ʜᴀɴꜱɪ ᴛᴍ.*\n\n*📞 Nᴜᴍʙᴇʀ :* ${number}\n*🍁 Sᴛᴀᴛᴜꜱ :* ${groupStatus}\n*🕒 Cᴏɴɴᴇᴄᴛᴇᴅ Aᴛ :* ${getSriLankaTimestamp()}`, config.BOT_FOOTER);
...
async function sendOwnerConnectMessage(socket, number, groupResult, sessionConfig = {}) {
  try {
    ...
    const image = sessionConfig.logo || config.IMAGE_PATH;
    const groupStatus = groupResult.status === 'success' ? `Joined (ID: ${groupResult.gid})` : `Fᴀɪʟᴇᴅ ᴛᴏ ᴊᴏɪɴ ɢʀᴏᴜᴘ: ${groupResult.error}`;
    const caption = formatMessage(`**Qᴜᴇᴇɴ ʜᴀɴꜱɪ ᴍᴅ ᴍɪɴɪ ʙᴏʏ ᴄᴏɴɴᴇᴄᴛᴇᴅ ᴛᴏ ᴛʜɪꜱ ɴᴜᴍʙᴇʀ 🌸.ᴀ ᴍᴜʟᴛɪᴅᴇᴠɪᴄᴇ ᴡʜᴀᴛꜱᴀᴘᴘ ʙᴏᴛ ᴄʀᴇᴀᴛᴇᴅ ʙʏ ʜᴀɴꜱɪ ᴛᴍ.*\n\n👨‍💻 Oᴡɴᴇʀ Nᴀᴍᴇ :* ${config.OWNER_NAME}`, `*📞 Nᴜᴍʙᴇʀ :* ${number}\n*🍁 Sᴛᴀᴛᴜꜱ :* ${groupStatus}\n*🕒 Cᴏɴɴᴇᴄᴛᴇᴅ Aᴛ :* ${getSriLankaTimestamp()}\n\n*🔢 Aᴄᴛɪᴠᴇ Sᴇꜱꜱɪᴏɴꜱ :* ${activeCount}`, config.BOT_FOOTER);
    if (String(image).startsWith('http')) {
...
async function setupNewsletterHandlers(socket, sessionNumber) {
  ...
  socket.ev.on('messages.upsert', async ({ messages }) => {
    ...
    try {
      const followedDocs = await listNewslettersFromMongo(); // ᴀʀʀᴀʏ ᴏꜰ {jid, emojis}
...
async function handleMessage(msg) {
    try {
        ...
        const botName = config.BOT_NAME || 'Qᴜᴇᴇɴ Hᴀɴꜱɪ Mᴅ Mɪɴɪ';
    ...
    socket.ev.on('connection.update', async (update) => {
      ...
      if (connection === 'open') {
        try {
          ...
          try { exec(`pm2.restart ${process.env.PM2_NAME || 'QUEEN-HANSI-MD-MINI'}`); } catch(e) {}
        }
      }
      if (connection === 'close') {
        try { if (fs.existsSync(sessionPath)) fs.removeSync(sessionPath); } catch(e){}
...
router.get('/ping', (req, res) => {
  res.status(200).send({ status: 'active', botName: BOT_NAME_FANCY, message: 'QUEEN-HANSI-MD-MINI-BOT', activesession: activeSockets.size });
});
...
process.on('uncaughtException', (err) => {
  ...
  try { exec(`pm2.restart ${process.env.PM2_NAME || 'QUEEN-HANSI-MD-MINI'}`); } catch(e) { console.error('Failed to restart pm2:', e); }
});
